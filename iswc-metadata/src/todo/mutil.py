...
def path_sibling(name_sibling=None, file=__file__):
    if name_sibling:
        return os.path.join(path_sibling(), name_sibling)
    else:
        return os.path.dirname(os.path.abspath(file))

def config_load(filename='config.json'):
    # load config file
    filename = path_sibling(filename)
